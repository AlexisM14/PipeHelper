...
def calculer_coordonnées_coude(x_depart, y_depart, longueur, angle_deg, direction):
    ...
    nbre_points_angle = longueur * 100
...
def calculer_coordonnees_guide(canalisation):
    ...
    for i in range(len(canalisation)):
        liste_longueur = np.append(liste_longueur, int(canalisation[i][0]))
        liste_forme = np.append(liste_forme, canalisation[i][4])
        liste_angle = np.append(liste_angle, int(canalisation[i][5]))
    ...
    for idx, i in enumerate(liste_forme):
        ...
        if i == 'droit':
            if idx > 0:
                if liste_forme[i-1][-1] == 'D':
                    for j in range(liste_longueur[idx]):
                        x_guide = np.append(x_guide, x_debut_troncon + j)
                        y_guide = np.append(y_guide, y_debut_troncon)

                elif liste_forme[i-1][-1] == 'G':
                    for j in range(liste_longueur[idx]):
                        x_guide = np.append(x_guide, x_debut_troncon - j)
                        y_guide = np.append(y_guide, y_debut_troncon)

                elif liste_forme[i-1][-1] == 'B':
                    for j in range(liste_longueur[idx]):
                        x_guide = np.append(x_guide, x_debut_troncon)
                        y_guide = np.append(y_guide, y_debut_troncon - j)

                elif liste_forme[i-1][-1] == 'H':
                    for j in range(liste_longueur[idx]):
                        x_guide = np.append(x_guide, x_debut_troncon)
                        y_guide = np.append(y_guide, y_debut_troncon + j)

        elif i == 'coude D' or i == 'coude G' or i == 'coude B' or i == 'coude H':
            x, y = calculer_coordonnées_coude(x_debut_troncon, y_debut_troncon, liste_longueur[idx],
                                              liste_angle[idx], i)
